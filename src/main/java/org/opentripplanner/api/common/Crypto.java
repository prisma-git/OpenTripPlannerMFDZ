...
public class Crypto {
  ...
  static {
    String secretKey = Optional
      .ofNullable(System.getenv("ENCRYPTION_SECRET_KEY"))
      .filter(s -> !s.trim().isEmpty())
      .orElseGet(() -> {
        LOG.error(
          "No environment variable ENCRYPTION_SECRET_KEY defined! Falling back on a random secret key. This means that your redirect URLs will be invalid after restarting OTP."
        );
  ...
  public static DecryptionResult decryptWithExpiry(String cipherText)
    throws GeneralSecurityException {
    String[] plainTextWithExpiry = decrypt(cipherText).split(separator);
    OffsetDateTime expiry = OffsetDateTime.ofInstant(
      Instant.ofEpochSecond(Long.parseLong(plainTextWithExpiry[1])),
      ZoneOffset.UTC
    );
